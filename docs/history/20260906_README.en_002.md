...
- **Multiple accounts per provider**: enable "Isolated session (multi-account)" on a service to give it its own browser profile and cookie store, so several accounts at the same provider can be monitored side by side without sharing sign-in state. Services without the toggle keep the shared profile — one login per domain as before.
- **Bonus usage resets** (Codex "Usage limit resets" / GLM "用量重置额度"): the card shows the available count and earliest expiry (yellow within 24 hours); GLM's per-entry expiry lives behind the "重置管理" dialog, which the scraper opens and closes automatically; expired entries are hidden.
- **Explicit English month-name date parsing**: year-less dates like `Oct 4, 9:57 AM` no longer rely on unreliable BCL parsing.

See the complete [v1.6.0 release notes](docs/releases/v1.6.0.md).
