...
Captured reset text is parsed into an **absolute timestamp** at scrape time (so relative values like "resets in 2 hours" never drift). Recognized formats include `重置时间：16:33`, `2026-07-26 10:00`, `07-23 14:44 后重置`, `5天11时56分钟后刷新`, `Refreshes in 2 hours 30 mins`, `Resets Jul 29, 2026 1:04 AM`, and more; unparseable text is shown as-is. For Volcengine Coding Plan, an idle current session reports that its 5-hour timer starts on first use instead of inventing a reset timestamp.
