...
- **More reliable sign-in persistence**: session cookies are saved after every scrape outcome, when the login window closes, and on graceful exit, protected with Windows DPAPI and atomic replacement. UTC expiry handling is corrected. Provider-side expiry still requires signing in again.
- **Updated Volcengine Coding Plan support**: restores refresh timing for Current session / Last week / Last month, including compact text such as `5天11时56分钟后刷新`; an unused 5-hour session explains when its timer starts.
- **Shared 5-hour idle state**: any zero-usage 5-hour quota without reset information shows `No calls yet…`; a real countdown is never replaced.
- **Desktop polish**: a clearer Acrylic tint, a synchronized title-bar pin control, and bounded session saving during shutdown.

See the complete [v1.5.0 release notes](docs/releases/v1.5.0.md).
...
- Shared WebView2 user-data folder (`%LOCALAPPDATA%\AIQuotaMonitor\WebView2UserData`) — **one login per domain**; cookies (including session cookies) are saved after every scrape outcome, when the login window closes, and on graceful exit, using DPAPI encryption plus atomic replacement, then restored on startup
