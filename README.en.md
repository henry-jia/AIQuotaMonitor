...
- Right-click menu + system tray: refresh / pause / always-on-top / layout / language / settings / show-hide / exit; **single left click on the tray icon always summons** (covered or hidden, it shows and raises to front, never hides — hide via the title-bar button or the menus)
- **Taskbar presence** (default on, toggle in settings): while visible the widget takes a taskbar slot, so one click brings a covered window back; hiding to tray frees the slot
...
- Shared WebView2 user-data folder (`%LOCALAPPDATA%\AIQuotaMonitor\WebView2UserData`) — **one login per domain**; a per-service **"Isolated session (multi-account)"** toggle gives that service its own profile and cookie store, so **multiple accounts at the same provider** can be monitored side by side (each signs in once); cookies (including session cookies) are saved after every scrape outcome, when the login window closes, and on graceful exit, using DPAPI encryption plus atomic replacement, then restored on startup
- **Bonus usage resets** (Codex "Usage limit resets", GLM "用量重置额度"): the card shows the available count and earliest expiry (turns yellow within 24 hours); GLM's per-entry expiry lives behind the "重置管理" dialog, which the scraper opens and closes automatically; expired entries are hidden
- **English / 中文 UI** (follows system by default, quick-switch via right-click menu)

## Quick start

1. [Download the latest `AIQuotaMonitor.exe`](https://github.com/henry-jia/AIQuotaMonitor/releases/latest/download/AIQuotaMonitor.exe) (single self-contained exe, no .NET runtime needed; requires WebView2 Runtime, which ships with Windows 11). Once the winget package is approved: `winget install henry-jia.AIQuotaMonitor`.
...
| Value is remaining % | Optional. When the page shows "remaining" instead of "used" (e.g. `73% remaining` → shows 27% used), percent type only. Values explicitly labeled left/remaining are converted automatically — no need to enable |
...
Captured reset text is parsed into an **absolute timestamp** at scrape time (so relative values like "resets in 2 hours" never drift). Recognized formats include `重置时间：16:33`, `2026-07-26 10:00`, `07-23 14:44 后重置`, `5天11时56分钟后刷新`, `Refreshes in 2 hours 30 mins`, `Resets Jul 29, 2026 1:04 AM`, and more; unparseable text is shown as-is. Any zero-usage 5-hour quota without reset information reports that its timer starts on first use; a real countdown is never replaced.
...
3. **Session cookies vs persistent cookies.** Aliyun's login ticket is session-scoped — Chromium keeps it in memory only, gone on process exit (your daily browser never notices because its process lives for weeks). Since v1.5.0, the app exports cookies after every scrape outcome, when the login window closes, and on graceful exit; it DPAPI-encrypts and atomically saves them for startup restore. Server-side session expiry still requires re-login, nothing can fix that client-side.
...
- If numbers exist but parsing fails, adjust the regex so the first capture group is the value; values explicitly labeled left/remaining are converted to used automatically — enable "Value is remaining percent" only when no such wording is present;
...
Sessions live in `%LOCALAPPDATA%\AIQuotaMonitor\WebView2UserData`, one login per domain. Cookies (including session cookies) are saved after every scrape outcome, when the login window closes, and on graceful exit; they are DPAPI-encrypted to `%LOCALAPPDATA%\AIQuotaMonitor\cookies.dat` using atomic replacement and restored on startup. Services with "isolated session" enabled use their own profile under `Sessions\<serviceId>` and their own `cookies-<serviceId>.dat`. The app never extends a server-issued expiry, so server-side session expiry still requires re-login (a tray balloon tells you).
...
`config.json` lives **next to the exe** (`System.Text.Json`, camelCase): services & rules (each service carries a stable `id` used to correlate history), window position, always-on-top, taskbar presence, opacity, layout, theme, language, refresh intervals, pause states. No hand-editing needed — dragging the window, toggling options, and reordering cards all save automatically.
...
- This tool is for **checking your own accounts' usage**: it opens vendor pages with your local login, parses and displays locally, and never uploads authentication or usage data. To preserve sign-in and display continuity it keeps the WebView2 profile locally and saves cookies with Windows user-bound DPAPI encryption; `lastgood.json` and `history.jsonl` hold the last successful results and optional usage history. Respect each vendor's terms of service; don't use it for bulk scraping, automation abuse, or anything against the target sites' rules.
